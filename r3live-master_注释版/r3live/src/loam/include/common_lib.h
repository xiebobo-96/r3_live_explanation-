...
struct Camera_Lidar_queue
{
    ...
    std::deque<sensor_msgs::PointCloud2::ConstPtr> *m_liar_frame_buf = nullptr; //最终会指向lidar_buffer
    ...
    double imu_in(const double in_time)
    {
        ...
        {
            m_first_imu_time = in_time;// m_first_imu_time确定后就不会更改了
        }
    ...
    int lidar_in(const double &in_time)
    {
        ...
        {
            m_if_have_lidar_data = 1;  //只要有雷达数据就会被置1
    ...
    {
        return m_last_visual_time + m_camera_imu_td;// m_camera_imu_td恒为零吗?
    }

    bool if_camera_can_process()
    {
        m_if_have_camera_data = 1;
        double cam_last_time = get_camera_front_time();//当前正在处理的图像帧的时间戳
        double lidar_last_time = get_lidar_front_time();    //m_last_lidar_time 为m_liar_frame_buf(liar_buffer)中front的时间戳
...
struct MeasureGroup // Lidar data and imu dates for the curent process
{
    ...
    PointCloudXYZINormal::Ptr lidar; //装在lidar数据
    std::deque<sensor_msgs::Imu::ConstPtr> imu;//装载IMU数据
};
...
public:
    ...
    Eigen::Matrix<double, DIM_OF_STATES, DIM_OF_STATES> cov; // states covariance  //激光雷达共用?
    double last_update_time = 0;
    double          td_ext_i2c;    // td_ext_i2c_delta,td_ext_i2c在构造中一直被设置为0,在整个系统运行中都不会改变
    StatesGroup()
    {
