...
void ImuProcess::IMU_Initial( const MeasureGroup &meas, StatesGroup &state_inout, int &N )
{
    ...
    // TODO: fix the cov 
    //只改变了mean_acc与 mean_gyr ,而cov_acc与cov_gyr没有改变
    cov_acc = Eigen::Vector3d( COV_START_ACC_DIAG, COV_START_ACC_DIAG, COV_START_ACC_DIAG );
    cov_gyr = Eigen::Vector3d( COV_START_GYRO_DIAG, COV_START_GYRO_DIAG, COV_START_GYRO_DIAG );
    state_inout.gravity = Eigen::Vector3d( 0, 0, 9.805 );
    state_inout.rot_end = Eye3d;//3*3的单位矩阵
...
void ImuProcess::lic_state_propagate( const MeasureGroup &meas, StatesGroup &state_inout )
{
    ...
    double        end_pose_dt = pcl_end_time - imu_end_time; //最后的激光点与imu时间戳的差值,这里的时间应该小于零才对!

    state_inout = imu_preintegration( state_inout, v_imu, end_pose_dt );//更新StatesGroup类对象state_inout的协方差cov,并且提供了先验估计
...
StatesGroup ImuProcess::imu_preintegration( const StatesGroup &state_in, std::deque< sensor_msgs::Imu::ConstPtr > &v_imu, double end_pose_dt )
{
    ...
    Eigen::Vector3d acc_imu( 0, 0, 0 ), angvel_avr( 0, 0, 0 ), acc_avr( 0, 0, 0 ), vel_imu( 0, 0, 0 ), pos_imu( 0, 0, 0 ); //acc_imu是去重力加速度的加速度,acc_avr没有去重力加速度
    ...
    for ( std::deque< sensor_msgs::Imu::ConstPtr >::iterator it_imu = v_imu.begin(); it_imu != ( v_imu.end() - 1 ); it_imu++ )
    {
        ...
        {
            if_first_imu = 0;
            //这里很重要,这里计算了与上一激光帧最后一个激光点(或者上一图像帧)之间的时间差dt,此dt保证了IMU积分的连续性.
            dt = tail->header.stamp.toSec() - state_inout.last_update_time;//  g_lio_state.last_update_time = Measures.lidar_end_time
        }
        else
        {
        ...
        acc_avr_skew << SKEW_SYM_MATRIX( acc_avr );//SKEW_SYM_MATRIX()函数是什么意思,应该是反对称矩阵.
        ...
        cov_w.block< 3, 3 >( 0, 0 ) = Jr_omega_dt * cov_omega_diag * Jr_omega_dt * dt * dt;//ng
        cov_w.block< 3, 3 >( 3, 3 ) = R_imu * cov_gyr_diag * R_imu.transpose() * dt * dt;//①
        cov_w.block< 3, 3 >( 6, 6 ) = cov_acc_diag * dt * dt;//②  ①与②两项有问题,按照论文里的定义, cov_w.block< 3, 3 >( 3, 3 )=0,cov_w.block< 3, 3 >( 6, 6 )= -R_imu * cov_acc_diag * R_imu.transpose() * dt * dt.
    ...
    state_inout.last_update_time = v_imu.back()->header.stamp.toSec() + dt;//若与积分关于图像帧,则last_update_time是图像的时间戳;若关于激光,则last_update_time是激光帧最后一个激光点的时间戳.
    ...
    state_inout.pos_end = pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt; //图像帧的状态,或最后一个激光点状态更新
    ...
    if ( 0 )
    {
        ...
        check_in_out_state( state_in, state_inout );//对于state_inout.pos_end是否异常的一些操作
    }
...
void ImuProcess::lic_point_cloud_undistort( const MeasureGroup &meas, const StatesGroup &_state_inout, PointCloudXYZINormal &pcl_out )
{
    ...
    */
    ...
    Eigen::MatrixXd cov_w( Eigen::Matrix< double, DIM_OF_STATES, DIM_OF_STATES >::Zero() );
    double          dt = 0;
    for ( auto it_imu = v_imu.begin(); it_imu != ( v_imu.end() - 1 ); it_imu++ ) //( v_imu.end() - 1 )表是第几个?
    {
        auto &&head = *( it_imu );
        auto &&tail = *( it_imu + 1 );
        ...
        angvel_avr << 0.5 * ( head->angular_velocity.x + tail->angular_velocity.x ), 0.5 * ( head->angular_velocity.y + tail->angular_velocity.y ),
            0.5 * ( head->angular_velocity.z + tail->angular_velocity.z );
        acc_avr << 0.5 * ( head->linear_acceleration.x + tail->linear_acceleration.x ),                                                                                                                                //相当于求两帧IMU间的均值
            0.5 * ( head->linear_acceleration.y + tail->linear_acceleration.y ), 0.5 * ( head->linear_acceleration.z + tail->linear_acceleration.z );

        angvel_avr -= state_inout.bias_g;//去除零偏的陀螺仪角速度
        acc_avr = acc_avr - state_inout.bias_a;//去除零偏的加速度计加速度

#ifdef DEBUG_PRINT
// fout<<head->header.stamp.toSec()<<" "<<angvel_avr.transpose()<<" "<<acc_avr.transpose()<<std::endl;
#endif
        dt = tail->header.stamp.toSec() - head->header.stamp.toSec();  //两帧IMU间的时间差
        /* covariance propagation */
...
// fout<<head->header.stamp.toSec()<<" "<<angvel_avr.transpose()<<" "<<acc_avr.transpose()<<std::endl;
        ...
        IMU_pose.push_back( set_pose6d( offs_t, acc_imu, angvel_avr, vel_imu, pos_imu, R_imu ) ); //offt_t是与第一个激光点的时间差
    }
    //上面for循环就完成了IMU的积分传递
    /*** calculated the pos and attitude prediction at the frame-end ***/
    dt = pcl_end_time - imu_end_time; //似乎dt = -pcl_end_time + imu_end_time,因为最后一个IMU的时间应该大于最后一个激光点的时间?
    state_inout.vel_end = vel_imu + acc_imu * dt;
    state_inout.rot_end = R_imu * Exp( angvel_avr, dt );                                    // state_inout.vel_end, state_inout.rot_end, state_inout.pos_end表示的是最后一个激光点的位姿与速度,这里还没弥补最后一个IMU数据与最后一个激光点数据的时间差.
    state_inout.pos_end = pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt;

    Eigen::Vector3d pos_liD_e = state_inout.pos_end + state_inout.rot_end * Lidar_offset_to_IMU;//Lidar_offset_to_IMU固定不变
    ...
    std::cout << "propagated cov: " << state_inout.cov.diagonal().transpose() << std::endl;   //这里state_inout.cov并未发生改变
#endif

    /*** undistort each lidar point (backward propagation) ***/
    auto it_pcl = pcl_out.points.end() - 1;//最后一个激光点
    for ( auto it_kp = IMU_pose.end() - 1; it_kp != IMU_pose.begin(); it_kp-- )//从最后一个位姿开始,遍历到第二个位子,last_ium的位姿不要.
    {
        auto head = it_kp - 1;
        ...
        {
            dt = it_pcl->curvature / double( 1000 ) - head->offset_time;  //这里得offset_time与LiDAR_front_end.cpp中的offset_time不一样,这里的是IMU数据与第一个激光点的时间戳.
            ...
            Eigen::Vector3d T_ei( pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt + R_i * Lidar_offset_to_IMU - pos_liD_e );//R_i * Lidar_offset_to_IMU - pos_liD_e ,这个补偿项有什么用

            Eigen::Vector3d P_i( it_pcl->x, it_pcl->y, it_pcl->z );
            Eigen::Vector3d P_compensate = state_inout.rot_end.transpose() * ( R_i * P_i + T_ei ); //将当前激光帧所有点都转换到当前激光帧最后一个点的IMU坐标系下.
...
void ImuProcess::Process( const MeasureGroup &meas, StatesGroup &stat, PointCloudXYZINormal::Ptr cur_pcl_un_ )
{
    ...
    if ( imu_need_init_ )
    {
        ...
        IMU_Initial( meas, stat, init_iter_num );//初始化 state_inout.gravity = Eigen::Vector3d( 0, 0, 9.805 );   state_inout.rot_end = Eye3d;state_inout.bias_g = mean_gyr;
    ...
    else
    {
        if ( 1 )
        {
            lic_point_cloud_undistort( meas, stat, *cur_pcl_un_ );//此函数去除点云畸变,meas, stat不会改变,因为形参加了const,只有cur_pcl_un_发生改变
        }
        else
        {
            *cur_pcl_un_ = *meas.lidar;
        }
        lic_state_propagate( meas, stat ); //meas不变,但是 stat改变 ,实现预积分的传播与stat的更新
    }
    // t2 = omp_get_wtime();

    last_imu_ = meas.imu.back();//是否与上面重复了?
